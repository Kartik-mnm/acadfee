...
// ✅ Use verified domain — delivers to any email worldwide
...
async function sendReceiptEmail(payment) {
  ...
  // Use the academy's actual name (not hardcoded Nishchay)
...
<div style="max-width:560px;margin:30px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.1);">
  ...
  <div style="background:#1a1f35;padding:16px 32px;text-align:center;">
    <div style="font-size:12px;color:rgba(255,255,255,0.5);">${academyName.toUpperCase()} · Powered by Exponent</div>
  </div>
</div></body></html>`;
...
async function sendFeeSummaryEmail({ student, fees, payments, attendance, tests, academy }) {
  ...
  // Use the academy's actual name dynamically
...
<div style="max-width:600px;margin:30px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.1);">
  ...
  <div style="padding:28px 32px;">
    ...
    <div style="text-align:center;padding-top:16px;border-top:1px solid #eee;font-size:12px;color:#888;">This is an automated email from ${academyName}${contactPhone ? " · Contact: " + contactPhone : ""} · Powered by Exponent</div>
  </div>
