...
// GET /api/daily-report/data?date=YYYY-MM-DD  → JSON
// GET /api/daily-report/excel?date=YYYY-MM-DD → .xlsx download
// GET /api/daily-report/print?date=YYYY-MM-DD → printable HTML page
// POST /api/daily-report/email               → emails report to academy admin
...
// ── Fetch all today's activity for an academy ────────────────────────────
  ...
  const [academy, payments, newStudents, attendance, expenses, feeDue] = await Promise.all([
    ...
    db.query(`
      ...
      WHERE s.academy_id = $1
        AND DATE(p.created_at AT TIME ZONE 'Asia/Kolkata') = $2
      ORDER BY p.created_at DESC
    `, [academyId, d]),

    // New students added today
    db.query(`
    ...
    db.query(`
      ...
      WHERE br.academy_id = $1
        AND DATE(e.created_at AT TIME ZONE 'Asia/Kolkata') = $2
      ORDER BY e.created_at DESC
    `, [academyId, d]),

    // Fee records created/updated today (new dues generated)
    db.query(`
      ...
      WHERE s.academy_id = $1
        AND DATE(fr.created_at AT TIME ZONE 'Asia/Kolkata') = $2
    ...
    const addSheet = (name, columns, rows) => {
      ...
      rows.forEach((row, i) => {
        ...
        columns.forEach((c, ci) => {
          if (c.format === "money") r.getCell(ci + 1).numFmt = MONEY;
...
// ── GET /api/daily-report/print — returns a printable HTML page (acts as PDF) ──
...
<table>${thead(["Receipt","Student","Branch","Batch","Period","Amount","Mode"])}<tbody>
${data.payments.map(p => row([p.receipt_no, p.student_name, p.branch_name, p.batch_name || "—", p.period_label || "—", fmt(p.amount), p.payment_mode])).join("")}
</tbody></table>`}
...
// ── POST /api/daily-report/email — emails today's report to the academy admin ──
router.post("/email", auth, async (req, res) => {
  try {
    ...
    const fmt  = (n) => `\u20b9${parseFloat(n || 0).toLocaleString("en-IN", { minimumFractionDigits: 2 })}`;
    const row  = (cells) => `<tr>${cells.map((c, i) => `<td style="padding:6px 10px;border-bottom:1px solid #eee;${i===0?"font-weight:600":""};">${c ?? ""}</td>`).join("")}</tr>`;
    ...
    const paymentsTable = `
      ...
      ${data.payments.map(p => row([p.student_name, p.branch_name, p.period_label || "—", fmt(p.amount), p.payment_mode])).join("")}`;
    ...
    const studentsTable = `
      ...
      ${data.new_students.map(st => row([st.name, st.phone, st.branch_name, st.batch_name || "—"])).join("")}`;
    ...
    const expensesTable = `
      ...
      ${data.expenses.map(e => row([e.description, e.category || "—", fmt(e.amount)])).join("")}`;
...
<div style="max-width:600px;margin:30px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.1);">
  ...
  <div style="padding:24px 32px;">
    ...
    <div style="margin-top:20px;padding:12px 16px;background:#f5f5ff;border-radius:8px;font-size:12px;color:#555;">
      \uD83D\uDCCE To download the full Excel report, log in to your academy dashboard → Reports → Download Today's Report.
    </div>
  </div>
