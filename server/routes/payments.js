...
router.post("/", auth, async (req, res) => {
  ...
  try {
    ...
    // FIX: only fee_record_id is required from frontend — we look up student_id from the fee record
    if (!fee_record_id || !amount || !payment_mode)
      return res.status(400).json({ error: "fee_record_id, amount and payment_mode are required" });

    // Fetch the fee record to get student_id and verify academy ownership
    ...
    // ── Generate meaningful sequential receipt number ──────────────────────────
    // Format: RCP-YYYYMMDD-NNNN  e.g. RCP-20260331-0001
    // Sequence is per-academy per-day to keep numbers short and human-readable
