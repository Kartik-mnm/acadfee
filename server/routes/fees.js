...
// Bulk generate monthly fee records — wrapped in transaction for consistency (#9)
router.post("/generate", auth, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "Fee generation failed — all changes rolled back" });
  } finally {
    client.release();
  }
});
...
router.patch("/mark-overdue", auth, async (req, res) => {
  ...
  try {
    ...
    console.error("Mark overdue error:", e.message);
