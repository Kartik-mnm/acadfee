...
router.get("/token/:student_id", auth, async (req, res) => {
  ...
  const token = jwt.sign({ student_id: rows[0].id, branch_id: rows[0].branch_id, type: "qr_attendance" }, getJwtSecret());
...
router.post("/scan", auth, async (req, res) => {
  ...
  } catch (e) { return res.status(400).json({ error: "Invalid or tampered QR code" }); }
  ...
  // #53 — Check if today is a working day
  ...
  if (scanRows.length === 0) {
    ...
    // #53 + #54 — Only update attendance if today is a working day
    if (isWorkingDay) {
      await db.query(
...
router.get("/today", auth, async (req, res) => {
  ...
  const { rows } = await db.query(
    `SELECT qs.*, s.name AS student_name, s.phone, b.name AS batch_name, br.name AS branch_name, u.name AS scanned_by_name
