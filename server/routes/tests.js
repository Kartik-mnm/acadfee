...
router.post("/", auth, async (req, res) => {
  ...
  try {
    ...
    const { rows } = await db.query(
      ...
      [bid, batch_id || null, name, subject || null, total_marks, test_date]
    );
    res.json(rows[0]);
  } catch (e) {
...
// Get results for a test — scoped to academy
