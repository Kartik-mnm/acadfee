...
router.get("/", auth, branchFilter, async (req, res) => {
  ...
  const { rows } = await db.query(
    `SELECT a.*, s.name AS student_name, s.phone, b.name AS batch_name, br.name AS branch_name,
            ROUND((a.present::numeric / NULLIF(a.total_days,0)) * 100, 1) AS percentage
     FROM attendance a
...
router.post("/", auth, async (req, res) => {
  const { student_id, month, year, total_days, present } = req.body;
  ...
  const { rows } = await db.query(
    `INSERT INTO attendance (student_id, branch_id, month, year, total_days, present)
     ...
     DO UPDATE SET total_days=$5, present=$6
     RETURNING *`,
...
// Bulk save attendance for a batch
router.post("/bulk", auth, async (req, res) => {
  const { records } = req.body; // [{student_id, total_days, present, month, year}]
  let saved = 0;
  for (const r of records) {
    ...
    await db.query(
      `INSERT INTO attendance (student_id, branch_id, month, year, total_days, present)
       ...
       ON CONFLICT (student_id, month, year)
       DO UPDATE SET total_days=$5, present=$6`,
      [r.student_id, sRows[0].branch_id, r.month, r.year, r.total_days, r.present]
    );
    saved++;
  }
  res.json({ saved });
