...
// Save / update attendance — validate present <= total_days
router.post("/", auth, async (req, res) => {
  let { student_id, month, year, total_days, present } = req.body;
  // Validate: present cannot exceed total_days
...
router.post("/bulk", auth, async (req, res) => {
  ...
  for (const r of records) {
    ...
    const present    = Math.min(parseInt(r.present) || 0, total_days); // Cap at total_days
