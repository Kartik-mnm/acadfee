      ...
      if (acad[0] && !acad[0].is_active) {
        ...
        return res.status(403).json({
          error: isExpiredTrial
            ? "Your 7-day free trial has expired. Please contact us to upgrade your plan."
...
router.post("/forgot-password", async (req, res) => {
  ...
  try {
    ...
    if (!rows[0]) return; // silently do nothing
    ...
    const resetUrl = `https://acadfee-app.onrender.com/reset-password?token=${token}`;
    await resend.emails.send({
      ...
      html: `
        <div style="font-family:Arial,sans-serif;max-width:500px;margin:20px auto;background:#0d1117;border-radius:12px;padding:32px;border:1px solid #1e2535;">
          ...
          <a href="${resetUrl}" style="display:inline-block;margin:20px 0;padding:12px 28px;background:linear-gradient(135deg,#6366f1,#a855f7);color:#fff;border-radius:8px;text-decoration:none;font-weight:700;">Reset Password</a>
          <p style="color:#454f72;font-size:12px;">If you didn't request this, ignore this email. Your password won't change.</p>
        </div>
      `,
    });
  } catch (e) {
...
router.post("/student-login", loginLimiter, async (req, res) => {
  ...
  try {
    ...
    res.json({
      token: accessToken, refreshToken,
      user: { id: student.id, name: student.name, email: student.email, role: "student", branch_id: student.branch_id, branch_name: student.branch_name, batch_name: student.batch_name, academy_id: student.academy_id || null },
    });
  } catch (e) {
...
router.delete("/student-sessions-all/:studentId", auth, async (req, res) => {
  ...
  try {
    const { rowCount } = await db.query(
      `DELETE FROM refresh_tokens WHERE (payload->>'id')::int=$1 AND payload->>'role'='student'`, [req.params.studentId]
    );
  ...
  try {
    const { rows } = await db.query(
      `SELECT u.id,u.name,u.email,u.role,u.branch_id,b.name AS branch_name
       ...
       WHERE u.academy_id=$1 ORDER BY u.id`,
...
router.delete("/users/:id", auth, superAdmin, async (req, res) => {
  ...
  try {
    const { rowCount } = await db.query(
      "DELETE FROM users WHERE id=$1 AND academy_id=$2", [targetId, req.user.academy_id || 0]
    );
...
router.patch("/users/:id/password", auth, superAdmin, async (req, res) => {
  try {
    ...
    const { rowCount } = await db.query(
      ...
      [hash, req.params.id, req.user.academy_id || 0]
    );
    if (rowCount === 0) return res.status(404).json({ error: "User not found in your academy" });
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
