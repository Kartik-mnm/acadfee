...
router.post("/student-login", loginLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!student)
      return res
        .status(401)
        .json({ error: "Invalid email or student portal not enabled" });
...
// Student logout — clear FCM token for this device only
...
// #2 — password_hint column REMOVED
...
// #2 — password_hint column REMOVED from SELECT
