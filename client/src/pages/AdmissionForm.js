...
// Resolve academy slug from URL query param ?slug=nishchay
// Falls back to localStorage, then "nishchay" as last resort
function resolveSlug() {
  const params = new URLSearchParams(window.location.search);
  const fromUrl = params.get("slug");
  if (fromUrl) return fromUrl;
  return localStorage.getItem("academy_slug") || "nishchay";
}
...
export default function AdmissionForm() {
  ...
  const printForm = () => {
    ...
    <style>
      * { margin:0; padding:0; box-sizing:border-box; }
      ...
      .header { padding: 20px 28px 16px; border-bottom: 4px solid ${accentColor}; }
      .academy-name { font-size: 32px; font-weight: 900; color: ${accentColor}; letter-spacing: 2px; line-height: 1; }
      .academy-addr { font-size: 11px; color: #333; margin-top: 8px; line-height: 1.8; text-align: center; }
      .form-title-bar { text-align: center; padding: 10px; border-bottom: 2px solid ${accentColor}; }
      .form-title-text { font-size: 17px; font-weight: 900; color: ${accentColor}; letter-spacing: 3px; text-decoration: underline; }
      ...
      .inp { width: 100%; padding: 3px 2px; border: none; border-bottom: 1.5px solid #333; background: transparent; font-size: 13px; }
      ...
      .addr-box { flex: 1; min-height: 55px; border: 1px solid #999; padding: 6px 8px; border-radius: 2px; font-size: 13px; white-space: pre-wrap; }
      .declaration { border: 2px solid #333; border-radius: 4px; padding: 14px 18px; margin-bottom: 18px; }
      .decl-title { text-align: center; font-size: 13px; font-weight: 900; color: ${accentColor}; text-decoration: underline; margin-bottom: 12px; letter-spacing: 1px; }
    ...
    <div class="form-wrap">
      ...
      <div class="body">
        <div class="top-row">
          ...
          <div class="photo-box">${form.photo_url ? `<img src="${form.photo_url}" style="width:100%;height:100%;object-fit:cover" />` : "PHOTO"}</div>
        </div>
    ...
    <div class="no-print" style="text-align:center;margin-top:20px">
      <button onclick="window.print()" style="padding:10px 28px;background:${accentColor};color:white;border:none;border-radius:6px;font-size:15px;cursor:pointer;font-weight:900">🖨 PRINT</button>
    </div>
    </body></html>`);
    w.document.close();
  };
  ...
  const inp = {
    width: "100%", padding: "5px 2px", border: "none", borderBottom: "1.5px solid #333",
    background: "transparent", fontSize: 13, outline: "none",
    fontFamily: "Arial, sans-serif", color: "#000", caretColor: "#000",
  };
    ...
    <div style={{ minHeight: "100vh", background: "#f5f5f5", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "white", borderRadius: 16, padding: "40px", textAlign: "center", maxWidth: 400, boxShadow: "0 4px 24px rgba(0,0,0,0.1)" }}>
        ...
        <div style={{ fontSize: 14, color: "#555", lineHeight: 1.7 }}>
          Thank you for registering at<br/><strong>{academyName}</strong>.<br/>Our team will contact you shortly.
        </div>
        {contactLine && (
          <div style={{ marginTop: 20, padding: "12px", background: "#f0f7ff", borderRadius: 8, fontSize: 13, color: accentColor }}>📞 {contactLine}</div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div style={{ background: "#e8e8e8", minHeight: "100vh", padding: "20px 16px" }}>
      <style>{`
        ...
        .admission-form input::placeholder { color: #999 !important; }
      ...
      <div className="admission-form" style={{ maxWidth: 720, margin: "0 auto", background: "white", boxShadow: "0 4px 32px rgba(0,0,0,0.18)", borderRadius: 4 }}>
        ...
        <div style={{ padding: "20px 28px 16px", borderBottom: `4px solid ${accentColor}` }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 20 }}>
            {academy?.logo_url ? (
              <img src={academy.logo_url} alt={academyName} style={{ width: 80, height: 80, objectFit: "contain", flexShrink: 0 }} />
            ) : (
              <div style={{ width: 72, height: 72, borderRadius: 12, background: accentColor, display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontSize: 32, fontWeight: 900, flexShrink: 0 }}>
        ...
        <div style={{ padding: "18px 28px" }}>
          ...
          {/* TOP: Aadhar + Branch + Course + Photo */}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12, gap: 20 }}>
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                <span style={lbl}>AADHAR NO. :</span>
                <input style={{ ...inp }} value={form.aadhar} onChange={(e) => f("aadhar", e.target.value)} />
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                <span style={{ ...lbl, minWidth: 60 }}>BRANCH :</span>
                <select style={{ ...inp, cursor: "pointer", background: "white" }} value={form.branch_id}
                  onChange={(e) => { f("branch_id", e.target.value); f("batch_id", ""); }}>
                  <option value="">Select Branch</option>
                  {branches.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}
                </select>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ ...lbl, minWidth: 60 }}>COURSE :</span>
                <select style={{ ...inp, cursor: "pointer", background: "white" }} value={form.batch_id} onChange={(e) => f("batch_id", e.target.value)}>
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
              ...
              >
                ...
                {uploadingPhoto && (
                  <div style={{ position: "absolute", inset: 0, background: "rgba(255,255,255,0.9)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontSize: 10, gap: 4 }}>
                    <div style={{ width: 20, height: 20, border: `2px solid ${accentColor}`, borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
                    Uploading…
                  </div>
                )}
              </div>
              ...
              {photoPreview && !uploadingPhoto && (
                <button type="button" onClick={() => { setPhotoPreview(""); f("photo_url", ""); }}
                  style={{ fontSize: 10, color: accentColor, background: "none", border: "none", cursor: "pointer", padding: 0 }}>Remove</button>
              )}
          ...
          {[{num:"1)",label:"FULL NAME OF STUDENT :",key:"name",required:true},{num:"2)",label:"FATHER NAME :",key:"father_name"},{num:"3)",label:"MOTHER NAME :",key:"mother_name"}].map((row) => (
            <div key={row.key} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
              ...
              <span style={{ ...lbl, minWidth: 190 }}>{row.label}{row.required && <span style={{ color: accentColor }}> *</span>}</span>
          ...
          <div style={{ display: "flex", gap: 16, marginBottom: 12, alignItems: "center" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 2 }}>
              ...
              <span style={{ ...lbl, minWidth: 130 }}>DATE OF BIRTH :</span>
              <input type="date" style={{ ...inp, flex: 1 }} value={form.dob} onChange={(e) => f("dob", e.target.value)} />
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1 }}>
              <span style={{ ...lbl, minWidth: 22 }}>5)</span>
              <span style={lbl}>AGE :</span>
              <input style={{ ...inp, width: 60 }} value={form.age} onChange={(e) => f("age", e.target.value)} placeholder="yrs" />
            </div>
          </div>
          ...
          {[{num:"6)",label:"MOTHER TONGUE :",key:"mother_tongue"},{num:"7)",label:"PREVIOUS SCHOOL :",key:"previous_school"}].map((row) => (
            <div key={row.key} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
              <span style={{ ...lbl, minWidth: 22 }}>{row.num}</span>
              <span style={{ ...lbl, minWidth: 190 }}>{row.label}</span>
              <input style={{ ...inp, flex: 1 }} value={form[row.key]} onChange={(e) => f(row.key, e.target.value)} />
            </div>
          ))}
          ...
          <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 12 }}>
            ...
            <textarea
              style={{ flex: 1, resize: "none", height: 55, border: "1px solid #999", padding: "6px 8px", borderRadius: 2, fontSize: 13, outline: "none", fontFamily: "Arial, sans-serif", color: "#000", background: "white" }}
          ...
          {[{num:"11)",label:"STUDENT MOBILE NUMBER :",key:"phone",required:true,type:"tel"},{num:"12)",label:"PARENT MOBILE NUMBER :",key:"parent_phone",required:true,type:"tel"},{num:"13)",label:"EMAIL ID :",key:"email",type:"email"}].map((row) => (
            <div key={row.key} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
              <span style={{ ...lbl, minWidth: 22 }}>{row.num}</span>
              <span style={{ ...lbl, minWidth: 220 }}>{row.label}{row.required && <span style={{ color: accentColor }}> *</span>}</span>
              <input type={row.type || "text"} style={{ ...inp, flex: 1 }} value={form[row.key]} onChange={(e) => f(row.key, e.target.value)} />
            </div>
          ))}
          ...
          <div style={{ border: "2px solid #333", borderRadius: 4, padding: "14px 18px", marginBottom: 18 }}>
            ...
            <div style={{ fontSize: 12, lineHeight: 2, color: "#111" }}>
              I <span style={{ display: "inline-block", minWidth: 180, borderBottom: "1px solid #333" }}>&nbsp;</span> REQUEST TO ADMIT MY SON / DAUGHTER IN CLASS <span style={{ display: "inline-block", minWidth: 60, borderBottom: "1px solid #333" }}>&nbsp;</span> OF {academyName.toUpperCase()}. I AGREE TO THE TERMS AND CONDITIONS OF THE INSTITUTE AND ASSURE TO ABIDE BY THEM. I ALSO UNDERTAKE TO PAY THE FEES LEVIED.
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 20 }}>
              <div style={{ fontSize: 12 }}>DATE : <span style={{ display: "inline-block", minWidth: 130, borderBottom: "1px solid #333" }}>&nbsp;</span></div>
              <div style={{ fontSize: 12 }}>SIGNATURE : ___________________</div>
            </div>
          </div>

          <button onClick={printForm} type="button" style={{ width: "100%", padding: "10px", background: "#0a1628", color: "white", border: "none", borderRadius: 6, fontSize: 14, fontWeight: 700, cursor: "pointer", letterSpacing: 1, marginBottom: 10 }}>
            🖨 Print This Form
          </button>

          <button onClick={submit} disabled={saving || uploadingPhoto}
            style={{ width: "100%", padding: "13px", background: (saving || uploadingPhoto) ? "#999" : accentColor, color: "white", border: "none", borderRadius: 6, fontSize: 15, fontWeight: 900, cursor: (saving || uploadingPhoto) ? "not-allowed" : "pointer", letterSpacing: 2 }}>
